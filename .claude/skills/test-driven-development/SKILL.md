...
## Human-in-the-Loop 매크로 사이클

아래 "Red-Green-Refactor" 절이 설명하는 것은 테스트 하나하나에 대한 **미시(micro) 사이클**이다. 그 미시 사이클을 감싸는 **매크로(macro) 사이클**로서, 기능/버그 단위 작업마다 다음 세 단계에서 사람 파트너의 승인을 받아야 한다. 어느 단계든 사람이 반려(reject)하면 그 단계로 되돌아가고, 승인 없이 다음 단계로 넘어가지 않는다.

```
[1] PLAN(RED)  --승인, Plan.md 커밋-->  [2] IMPLEMENT(GREEN)  --완료-->  [3] REVIEW  --승인, 코드 커밋--> 다음 [1]
     ^                                                                        |
     |                                        반려/추가 리팩토링 필요               |
     +----------------------------------------------------------------------+
```
...
- 이번에 구현할 동작(behavior)의 목표를 정확히 설정한다.
- 코드를 작성하기 전에 `Plan.md`를 생성해 무엇을, 왜, 어떻게 테스트할지 제시한다.
- **사람 파트너에게 `Plan.md` 검토를 요청하고, 승인을 받은 뒤에만 다음 단계로 넘어간다.** 반려되면 `Plan.md`를 고쳐 다시 검토받는다.
- **승인을 받으면 `Plan.md`를 커밋한다.** (예: `git add Plan.md && git commit -m "docs: plan <이번 목표>"`) 이 커밋은 실패하는 테스트나 프로덕션 코드를 포함하지 않는다 — 계획만 기록한다.
- 승인 후에는 그 계획 범위 안에서 아래 "Red-Green-Refactor" 미시 사이클(RED 실패 테스트 작성 → RED 검증)을 필요한 만큼 반복해 진행한다.

### [2] GREEN — 계획 실현 및 실제 통과 확인

- `Plan.md`에서 합의된 목표만 달성하도록 최소한의 코드를 구현한다. 계획에 없는 기능을 추가하지 않는다.
- 각 테스트마다 아래 "Red-Green-Refactor" 미시 사이클의 GREEN 검증을 실제로 실행해 통과를 직접 확인한다. 통과했다고 가정하지 말라.
- `Plan.md`의 목표를 모두 달성하면 REVIEW 단계로 넘어갈 준비가 된 것이다.
- **이 단계에서는 아직 커밋하지 않는다.** 커밋은 REVIEW 단계에서 사람 파트너의 승인을 받은 뒤에 한다.

### [3] REVIEW — 코드 리뷰 및 리팩토링 요청

- GREEN 단계에서 작성된 코드 전체를 사람 파트너와 함께 리뷰한다.
- `Plan.md`에 명시되지 않은 사항이 구현되지는 않았는지 확인한다. 발견되면 제거하거나 사람 파트너의 별도 승인을 받는다.
- 리팩토링이 필요하다고 판단되면 사람 파트너에게 리팩토링을 요청한다 (아래 "REFACTOR - 정리" 참고). 리팩토링 후에도 테스트는 계속 그린이어야 한다.
- **사람 파트너가 리뷰(및 리팩토링 결과)를 승인하면, 그 시점의 테스트+프로덕션 코드를 커밋한다.** (예: `git add <테스트 파일> <구현 파일> && git commit -m "feat: <이번 목표>"`) 승인 없이 커밋하지 않는다.
- 커밋까지 완료되어야 이번 작업이 완료된 것으로 보고, 다음 목표의 [1] RED(새 `Plan.md`)로 넘어간다.
...
- "TDD는 교조적, 나는 실용적"
- "이건 다른 경우인데..."
- "Plan.md 없이 바로 구현을 시작했다"
- "사람 파트너의 승인 없이 REVIEW를 건너뛰고 완료로 표시했다"
- "승인 없이 Plan.md나 코드를 커밋했다"
...
- [ ] `Plan.md`를 작성했고, 구현 전에 사람 파트너의 승인을 받았다
- [ ] 승인된 `Plan.md`를 커밋했다
...
- [ ] 사람 파트너와 REVIEW를 진행했고, `Plan.md` 범위를 벗어난 구현이 없음을 확인했다
- [ ] REVIEW 승인 후 테스트+프로덕션 코드를 커밋했다
